#!/bin/bash
#SBATCH --job-name=finsenti-qwen3.5-2b
...
#SBATCH --output=logs/qwen3_5_2b_%j.out
#SBATCH --error=logs/qwen3_5_2b_%j.err
#SBATCH --mail-user=ayshaikh@iu.edu
#SBATCH --mail-type=BEGIN,END,FAIL
#SBATCH --exclusive

# FinSenti - Qwen3.5-2B full pipeline (SFT -> GRPO -> Export)

set -euo pipefail
export PYTHONUNBUFFERED=1

echo "=== Job $SLURM_JOB_ID - Qwen3.5-2B ==="
...
# Upgrade Unsloth to pick up the PR #2673 save_pretrained_merged fix.
...
# Run full pipeline
python training/qwen3_5_2b.py --phase all --upload
