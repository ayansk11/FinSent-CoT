...
def _verify_export_roundtrip(merged_dir, system_prompt, min_tok_acc=0.10):
    """Reload the merged model FROM DISK (the real deployment path) and assert
    it round-trips. Guards against the MobileLLM/Llama4 full-FT corruption where
    trained weights don't survive save->reload (teacher-forced acc collapses to
    0.000 -> token salad). Raises RuntimeError so the SLURM job FAILS LOUDLY
    instead of uploading a broken model and wasting an eval cycle.

    Threshold is intentionally LOW (0.10): genuine capacity-floor models (e.g.
    Tiny-LLM at ~0.33 SFT acc) still pass and get uploaded as legitimate
    scaling-floor data points; only true corruption (salad ~0.00) is blocked.
    """
    import torch
    from transformers import AutoModelForCausalLM, AutoTokenizer
    ...
    model.train(False)  # inference mode (equivalent to .eval(), avoids hook)
    probes = [
    ...
    if acc < min_tok_acc:
        raise RuntimeError(
            ...
            f"This is the Llama4/trust_remote_code full-FT save bug -- use the PEFT path."
        )
...
def run_export(upload=False):
    ...
    # GATE: verify the saved model round-trips before any upload. Raises
    # RuntimeError (fails the job) if the reloaded model is salad.
