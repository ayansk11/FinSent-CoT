"""
FinSenti - Gemma3-270M: SFT -> GRPO -> Export -> Upload

Single self-contained script for the complete training pipeline.
Uses standard PEFT + bitsandbytes (kept consistent with MobileLLM/Tiny-LLM
PEFT path for architecture-compatibility safety - Unsloth Gemma-3 support
varies across releases). Export: merges PEFT adapters and saves HF weights.
GGUF conversion requires manual llama.cpp convert_hf_to_gguf.py.

Why this model: provides a sub-1B data point in the scaling story between
Tiny-LLM (10M, scaling floor) and Qwen3-0.6B (600M, smallest working model).
At 270M with a strong Google-trained base, expected to produce coherent
output well above the 10M floor.

Dataset: Ayansk11/FinSenti-Dataset (local validated splits)

Usage:
    python gemma3_270m.py --phase all          # Full pipeline
    python gemma3_270m.py --phase sft          # SFT only
    python gemma3_270m.py --phase grpo         # GRPO only
    python gemma3_270m.py --phase export       # Export only
"""
...
# Gemma 3 270M is Google's smallest text-only Gemma-3 instruct model. It's added
# to FinSenti to provide a meaningful sub-1B data point in the scaling story
# (sits between Tiny-LLM at 10M and Qwen3-0.6B at 600M). Trained via PEFT
# rather than Unsloth for architecture-compatibility safety.

MODEL_KEY = "gemma3-270m"
BASE_MODEL = "google/gemma-3-270m-it"
SHORT_NAME = "Gemma3-270M"
MODEL_FAMILY = "gemma3"
MAX_SEQ_LENGTH = 2048
TARGET_MODULES = [
...
# tensor (batch × seq × vocab) is the memory bottleneck, so batch is kept
# conservative; grad_accum brings effective batch back to 32.
SFT_BATCH_SIZE = 4
SFT_GRAD_ACCUM = 8
SFT_LR = 2e-4
SFT_LORA_R = 16
SFT_LORA_ALPHA = 32
SFT_EPOCHS = 5  # extra epochs for small model
...
HF_GGUF = "Ayansk11/FinSenti-Gemma3-270M-GGUF"
QUANTIZATIONS = ["Q4_K_M", "Q5_K_M", "Q8_0"]
MLX_REPOS = {
    4: "Ayansk11/FinSenti-Gemma3-270M-MLX-4bit",
    8: "Ayansk11/FinSenti-Gemma3-270M-MLX-8bit",
}
...
def _setup_pad_token(tokenizer, model):
    ...
    LoRA on lm_head crashes with a shape mismatch.
    """
    import torch
...
def _load_base_model_peft(base_model: str, lora_r: int, lora_alpha: int):
    ...
    # Untie FIRST so _setup_pad_token can correctly resize lm_head when it's
    # a separate tensor. If we resize first, transformers only touches input
    # embeddings (lm_head stays at old vocab), then LoRA-on-lm_head crashes.
...
def run_export(upload=False):
    ...
    # IMPORTANT: load the base model, resize embeddings to match the adapter's
    # vocab (the SFT/GRPO process added <|finsenti_pad|>), then load the
    # adapter. AutoPeftModelForCausalLM.from_pretrained does the loads in the
    # wrong order: it loads the base at its original vocab size first, then
    # tries to attach the adapter whose modules_to_save tensors are sized for
    # the new vocab, producing a size mismatch on lm_head + embed_tokens.
    # This pattern mirrors _load_peft_checkpoint() above.
    ...
    # lm_head delta isn't discarded when transformers re-ties on reload.
