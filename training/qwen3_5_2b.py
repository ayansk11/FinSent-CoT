"""
FinSenti - Qwen3.5-2B: SFT -> GRPO -> Export -> Upload
...
Usage:
    python qwen3_5_2b.py --phase all          # Full pipeline
    python qwen3_5_2b.py --phase sft          # SFT only
    python qwen3_5_2b.py --phase grpo         # GRPO only
    python qwen3_5_2b.py --phase export       # Export only
"""
...
# HuggingFace repos
HF_FULL = "Ayansk11/FinSenti-Qwen3.5-2B"
HF_GGUF = "Ayansk11/FinSenti-Qwen3.5-2B-GGUF"
QUANTIZATIONS = ["Q4_K_M", "Q5_K_M", "Q8_0"]
MLX_REPOS = {
    4: "Ayansk11/FinSenti-Qwen3.5-2B-MLX-4bit",
    8: "Ayansk11/FinSenti-Qwen3.5-2B-MLX-8bit",
}
...
def run_sft():
    ...
    # Workaround: Unsloth compiler can fail with OSError on Qwen3.5-2B
    # when inspect.getsource() can't find forward method source.
    # Retry with compilation disabled if it fails.
    try:
        ...
        if 'could not get source code' in str(e):
            print(f"  [Warn] Unsloth compile failed ({e}), retrying with compilation disabled...")
...
def run_export(upload=False):
    ...
    # with save_method="merged_16bit", but for this model (qwen3.5-2B) that
    # silently produces an upload where the LoRA adapter never folds into the
    # base, so the deployed model behaves like the untuned instruct base
    # (markdown-format output, no <reasoning>/<answer> tags). Loading in
    # bf16 forces the merge math to operate on full-precision base weights.
    model, tokenizer = FastLanguageModel.from_pretrained(
