"""
FinSenti — Qwen3.5-0.8B: SFT -> GRPO -> Export -> Upload
...
Usage:
    python qwen3_5_0_8b.py --phase all          # Full pipeline
    python qwen3_5_0_8b.py --phase sft          # SFT only
    python qwen3_5_0_8b.py --phase grpo         # GRPO only
    python qwen3_5_0_8b.py --phase export       # Export only
"""
...
# HuggingFace repos
HF_FULL = "Ayansk11/FinSenti-Qwen3.5-0.8B"
HF_GGUF = "Ayansk11/FinSenti-Qwen3.5-0.8B-GGUF"
QUANTIZATIONS = ["Q4_K_M", "Q5_K_M", "Q8_0"]
MLX_REPOS = {
    4: "Ayansk11/FinSenti-Qwen3.5-0.8B-MLX-4bit",
    8: "Ayansk11/FinSenti-Qwen3.5-0.8B-MLX-8bit",
}
    ...
    try:
        ...
        def _safe_3d(*a, **kw):
            try:
                ...
                # Fallback: return standard position_ids without 3D delta
                import torch as _t
                input_ids = a[0] if a else kw.get("input_ids")
                bs, seq = input_ids.shape
                return _t.arange(seq, device=input_ids.device).unsqueeze(0).expand(bs, -1)
