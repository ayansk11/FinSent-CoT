"""
FinSent-CoT — Qwen3-4B: SFT -> GRPO -> Export -> Upload
...
def run_sft():
    ...
    # ─── Load dataset (Ayansk11/FinSent-CoT-Dataset, local validated split) ─
...
def _patch_masked_batch_mean():
    """
    Fix Unsloth's tensor mismatch bug in masked_batch_mean.

    Unsloth's GRPOTrainer has a bug where completion tensor size can differ
    from completion_mask size (off by ~22 tokens). This patches ALL occurrences
    of masked_batch_mean across loaded modules to truncate to the shorter length.
    """
    import torch
    ...
    def _fixed_masked_batch_mean(x, completion_mask):
        ...
        completion_token_count = completion_mask.sum(dim=-1).clamp(min=1)
        return (x * completion_mask).sum(dim=-1) / completion_token_count

    patched = 0

    # Strategy 1: Search all loaded modules for masked_batch_mean attribute
    for mod_name, mod in list(sys.modules.items()):
        if mod is None:
            continue
        if hasattr(mod, 'masked_batch_mean'):
            setattr(mod, 'masked_batch_mean', _fixed_masked_batch_mean)
            patched += 1
            print(f"    Patched module: {mod_name}")

    # Strategy 2: Search function __globals__ in all modules
    for mod_name, mod in list(sys.modules.items()):
        if mod is None:
            continue
        for attr_name in dir(mod):
            try:
                ...
                for fn in [attr, getattr(attr, '__func__', None),
                           getattr(attr, '__wrapped__', None)]:
                    if fn is None:
                        continue
                    g = getattr(fn, '__globals__', None)
                    if g and 'masked_batch_mean' in g:
                        g['masked_batch_mean'] = _fixed_masked_batch_mean
                        patched += 1
            except Exception:
                pass

    print(f"  [Patch] masked_batch_mean: {patched} locations patched")
    return patched > 0
...
def run_grpo():
    ...
    # ─── Load GRPO dataset (Ayansk11/FinSent-CoT-Dataset) ────────────────
