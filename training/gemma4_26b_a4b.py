"""
FinSenti - Gemma4-26B-A4B: SFT -> GRPO -> Export -> Upload
...
Usage:
    python gemma4_26b_a4b.py --phase all          # Full pipeline
    python gemma4_26b_a4b.py --phase sft          # SFT only
    python gemma4_26b_a4b.py --phase grpo         # GRPO only
    python gemma4_26b_a4b.py --phase export       # Export only
"""
...
def _rebuild_grpo_config(d):
    """Pickle helper: reconstruct GRPOConfig from dict (avoids UnslothGRPOConfig identity issue)."""
...
# HuggingFace repos
HF_FULL = "Ayansk11/FinSenti-Gemma4-26B-A4B"
HF_GGUF = "Ayansk11/FinSenti-Gemma4-26B-A4B-GGUF"
QUANTIZATIONS = ["Q4_K_M", "Q5_K_M", "Q8_0"]
MLX_REPOS = {
    4: "Ayansk11/FinSenti-Gemma4-26B-A4B-MLX-4bit",
    8: "Ayansk11/FinSenti-Gemma4-26B-A4B-MLX-8bit",
}
...
def run_grpo():
    ...
    # Fix: Unsloth's dynamic UnslothGRPOConfig can't be serialized by torch.save
    # during checkpoint. Add custom reducer so it serializes as a plain dict
    # and reconstructs as a standard TRL GRPOConfig on load.
...
def run_export(upload: bool = False):
    ...
    # GGUF conversion via pre-built llama.cpp (bypasses Unsloth install_llama_cpp)
